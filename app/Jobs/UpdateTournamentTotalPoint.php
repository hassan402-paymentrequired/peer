...
class UpdateTournamentTotalPoint implements ShouldQueue
{
    use Queueable;
    ...
    {
        Log::info('Updating total point for tournament');

        $tournaments = Tournament::query()
            ->where('status', 'open')
            ->chunk(100, function ($tournaments) {
                $tournaments->each(function ($tournament) {
                    TournamentUser::with(['squads.mainPlayer', 'squads.subPlayer', 'user'])
                        ->where('tournament_id', $tournament->id)
                        ->chunk(300, function ($participants) {
                            Log::info("Calculating scores for the chunk of {$participants->count()} tournament participants");
                            $participants->each(function ($participant) {
                                ...
                                Log::info("Updated tournament participant {$participant->id} (User: {$participant->user_id}) with {$totalPoints} points");
                            });
                        });
                });
            });
    ...
    private function calculateParticipantScore($squads): int
    {
        ...
        foreach ($squads as $squad) {
            ...
            Log::debug("Tournament Squad points calculated", [
                'squad_id' => $squad->id,
    ...
    private function getPlayerPoints(int $playerId, ?int $playerMatchId): int
    {
        ...
        // Use the model's getPointsAttribute method or raw points
